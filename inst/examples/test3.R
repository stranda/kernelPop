...
locs <- as.matrix(data.frame(lft=c(0,501,rights[c(-1:-2)]-diff(rights[-1])+1),
  ...
  rland <- landscape.new.epoch(rland,S=S,R=R,M=M,
                     carry=(0.250 * (sqrt((locs[,3]-locs[,1])*(locs[,4]-locs[,2])))),
...
expmat <- matrix(c(1,0,0,
  ...
  hsq <- c(1,1,1)
  rland <- landscape.new.expression(rland,expmat=expmat*0.1,hsq=hsq)
  rland <- landscape.new.gpmap(rland,c(-1,0,-1,1,-1),c(-1,-1,2))
  initpopsize <- 500
